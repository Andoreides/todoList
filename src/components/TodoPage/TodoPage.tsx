...
const TodoPage = () => {
  const { inputValue, list, inputFilter, counter, loading } = useSelector(
    (store: StoreType) => store.todosState
  );
  ...
  const findToDo = () => {
    dispatch({
      type: "FIND_TODO",
      payload: list.filter((list) => list.todo === inputFilter),
    });
  };
    ...
    <div>
      <>
        ...
        <Input
          ...
          value={inputValue}
          onChange={(value) =>
            dispatch({ payload: value, type: CHANGE_INPUT_VALUE })
          }
        />
        <Input
          type="text"
          ...
          value={inputFilter}
          onChange={(value) =>
            dispatch({ payload: value, type: CHANGE_FILTER_VALUE })
          }
        />
